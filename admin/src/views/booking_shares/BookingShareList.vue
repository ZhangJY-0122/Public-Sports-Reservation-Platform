<template>
  <div class="booking-share-list">
    ...
    <!-- 操作工具栏 -->
    <div class="toolbar">
      <el-button type="primary" @click="handleAdd">新增分摊</el-button>
      ...
      <el-form :model="searchForm" class="search-form" inline>
        <el-form-item label="预约ID">
          <el-input v-model="searchForm.booking_id" placeholder="请输入预约ID"></el-input>
        </el-form-item>
    ...
    >
      ...
      <el-table-column prop="booking_id" label="预约ID" width="100"></el-table-column>
      <el-table-column prop="user_id" label="用户ID" width="100"></el-table-column>
      <el-table-column prop="share_amount" label="分摊金额" width="120">
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog 
      :title="dialogMode === 'add' ? '新增分摊' : '编辑分摊'" 
      v-model="dialogVisible"
      width="600px"
      @close="resetForm"
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        <el-form-item label="预约ID" prop="booking_id">
          <el-input-number v-model="form.booking_id" :min="1"></el-input-number>
        </el-form-item>
        <el-form-item label="用户ID" prop="user_id">
          <el-input-number v-model="form.user_id" :min="1"></el-input-number>
        </el-form-item>
        <el-form-item label="分摊金额" prop="share_amount">
          <el-input-number v-model="form.share_amount" :min="0" :precision="2"></el-input-number>
        </el-form-item>
        <el-form-item label="已付金额" prop="paid_amount">
          <el-input-number v-model="form.paid_amount" :min="0" :precision="2"></el-input-number>
        </el-form-item>
    ...
    <!-- 查看对话框 -->
    <el-dialog title="分摊详情" v-model="viewDialogVisible" width="600px">
      <div v-if="currentItem" class="view-content">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="ID">{{ currentItem.id }}</el-descriptions-item>
          <el-descriptions-item label="预约ID">{{ currentItem.booking_id }}</el-descriptions-item>
          <el-descriptions-item label="用户ID">{{ currentItem.user_id }}</el-descriptions-item>
          <el-descriptions-item label="分摊金额">¥{{ currentItem.share_amount }}</el-descriptions-item>
          <el-descriptions-item label="已付金额">¥{{ currentItem.paid_amount }}</el-descriptions-item>
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await fetch('/api/booking_shares/list', {
      method: 'GET',
      headers: {
    ...
    if (result.code === 200) {
      tableData.value = result.data.booking_shares || []
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除这个分摊记录吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    const response = await fetch(`/api/booking_shares/${row.id}`, {
      method: 'DELETE'
    })
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedItems.value.length} 个分摊记录吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    ...
    const response = await fetch('/api/booking_shares/batch_delete', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const url = dialogMode.value === 'add' ? '/api/booking_shares' : `/api/booking_shares/${form.id}`
