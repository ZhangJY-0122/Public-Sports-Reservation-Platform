<template>
  <div class="coach-booking-list">
    ...
    <div class="toolbar">
      ...
      <el-form :model="searchForm" class="search-form" inline>
        ...
        <el-form-item label="教练ID">
          <el-input v-model="searchForm.coach_id" placeholder="请输入教练ID"></el-input>
        </el-form-item>
    ...
    >
      ...
      <el-table-column prop="coach_name" label="教练ID" width="100"></el-table-column>
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog 
      :title="dialogMode === 'add' ? '新增教练预约' : '编辑教练预约'" 
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="教练ID" prop="coach_id">
          <el-input-number v-model="form.coach_id" :min="1"></el-input-number>
        </el-form-item>
    ...
    <!-- 查看对话框 -->
    <el-dialog title="教练预约详情" v-model="viewDialogVisible" width="600px">
      <div v-if="currentItem" class="view-content">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="教练ID">{{ currentItem.coach_id }}</el-descriptions-item>
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await request.get('/api/coaches/admin/bookings', params)
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除这个教练预约吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    const response = await fetch(`/api/coach_bookings/${row.id}`, {
      method: 'DELETE'
    })
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedItems.value.length} 个教练预约吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    ...
    const response = await fetch('/api/coach_bookings/batch_delete', {
      method: 'POST',
      headers: {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const url = dialogMode.value === 'add' ? '/api/coach_bookings' : `/api/coach_bookings/${form.id}`
