<template>
  <div class="friend-list">
    ...
    <!-- 操作工具栏 -->
    <div class="toolbar">
      <el-button type="primary" @click="handleAdd">新增朋友关系</el-button>
      ...
      <el-form :model="searchForm" class="search-form" inline>
        ...
        <el-form-item label="朋友用户ID">
          <el-input v-model="searchForm.friend_user_id" placeholder="请输入朋友用户ID"></el-input>
        </el-form-item>
    ...
    >
      ...
      <el-table-column prop="friend_user_id" label="朋友用户ID" width="120"></el-table-column>
      <el-table-column prop="status" label="状态" width="100">
        <template #default="{ row }">
          {{ getStatusText(row.status) }}
        </template>
      </el-table-column>
      <el-table-column prop="message" label="消息" width="200"></el-table-column>
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog 
      :title="dialogMode === 'add' ? '新增朋友关系' : '编辑朋友关系'" 
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        ...
        <el-form-item label="朋友用户ID" prop="friend_user_id">
          <el-input v-model="form.friend_user_id" placeholder="请输入朋友用户ID"></el-input>
        </el-form-item>
        ...
        <el-form-item label="消息" prop="message">
          <el-input v-model="form.message" type="textarea" :rows="3" placeholder="请输入消息"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 查看对话框 -->
    <el-dialog title="朋友关系详情" v-model="viewDialogVisible" width="600px">
      <div v-if="currentItem" class="view-content">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="朋友用户ID">{{ currentItem.friend_user_id }}</el-descriptions-item>
          <el-descriptions-item label="状态">{{ getStatusText(currentItem.status) }}</el-descriptions-item>
          <el-descriptions-item label="消息">{{ currentItem.message }}</el-descriptions-item>
...
const rules = {
  ...
  friend_user_id: [
    { required: true, message: '请输入朋友用户ID', trigger: 'blur' }
  ],
  status: [
...
const fetchData = async () => {
  ...
  try {
    ...
    if (result.code === 200) {
      tableData.value = result.data.friends || []
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定要删除这个朋友关系吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    const response = await fetch(`/api/friends/${row.id}`, {
      method: 'DELETE'
    })
...
const handleBatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`确定要删除选中的 ${selectedItems.value.length} 个朋友关系吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    ...
    const response = await fetch('/api/friends/batch_delete', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const url = dialogMode.value === 'add' ? '/api/friends' : `/api/friends/${form.id}`
